...
ULensFile* ULensSetupUtility::CreateLensFile(const FLensConfiguration& Config)
{
	...
	if (Config.bCreateNewLensFile || !LensFile)
	{
		...
		FString PackageName = FString::Printf(TEXT("/Game/TrackingAutoSetup/%s"), *Config.LensFileName);
		...
		if (LensFile)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Created lens file '%s' in package '%s'"),
	...
	else if (LensFile)
	{
		...
		LensFile->MarkPackageDirty();
		UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Updated existing lens file '%s'"), *LensFile->GetName());
	}
...
void ULensSetupUtility::PopulateEncoderTable(ULensFile* LensFile, const FLensConfiguration& Config)
{
	...
	for (int32 i = 0; i < NumFocusPoints; i++)
	{
		...
		UE_LOG(LogTemp, Verbose, TEXT("TrackingAutoSetup: Focus encoder point %d: raw=%.0f -> physical=%.1f cm"),
	...
	for (int32 i = 0; i < NumZoomPoints; i++)
	{
		...
		UE_LOG(LogTemp, Verbose, TEXT("TrackingAutoSetup: Zoom encoder point %d: raw=%.0f -> physical=%.1f mm"),
			i, RawValue, PhysicalValue);
	}

	UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Populated encoder table with %d focus points and %d zoom points"),
...
void ULensSetupUtility::PopulateFocalLengthTable(ULensFile* LensFile, const FLensConfiguration& Config)
{
	...
	for (int32 i = 0; i < NumPoints; i++)
	{
		...
		UE_LOG(LogTemp, Verbose, TEXT("TrackingAutoSetup: Focal length point %d: focus=%.1f zoom=%.1f Fx=%.3f Fy=%.3f"),
	...
	UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Populated focal length table with %d points"), NumPoints);
}
...
void ULensSetupUtility::PopulateImageCenterTable(ULensFile* LensFile, const FLensConfiguration& Config)
{
	...
	UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Populated image center table with default center point"));
}
...
void ULensSetupUtility::ConfigureCineCamera(UCineCameraComponent* Camera, const FLensConfiguration& Config)
{
	...
	UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Configured CineCamera - Sensor: %.1fx%.1f mm, Focal: %.1f-%.1f mm, Focus: %.0f-%.0f cm"),
...
void ULensSetupUtility::ConfigureLensComponent(ULensComponent* LensComp, ULensFile* LensFile, bool bUseLiveLink)
{
	...
	UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Configured LensComponent with lens file '%s', LiveLink: %s"),
...
TArray<ULensFile*> ULensSetupUtility::FindAllLensFiles()
{
	...
	UE_LOG(LogTemp, Log, TEXT("TrackingAutoSetup: Found %d lens file assets"), Result.Num());
	return Result;
}
